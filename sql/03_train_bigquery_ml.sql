-- Train an interpretable baseline and nonlinear challenger. Replace YOUR_PROJECT_ID.
...
FROM `YOUR_PROJECT_ID.ecommerce_analytics.session_purchase_features`
...
FROM ML.EVALUATE(MODEL `YOUR_PROJECT_ID.ecommerce_analytics.purchase_logistic`,
  (SELECT * EXCEPT(session_id,session_date) FROM `YOUR_PROJECT_ID.ecommerce_analytics.session_purchase_features`
